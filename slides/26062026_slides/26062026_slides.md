...
style: |
  ...
  .nav { text-align: center; font-size: 11.5px; font-weight: 600; letter-spacing: 0.2px; }
  ...
  section.title .subtitle { font-size: 18px; color: #475569; margin: 0 0 60px 28px; }
  section.title .meta { font-size: 13px; color: #64748b; margin-left: 28px; letter-spacing: 0.3px; }
---
...
<!-- _footer: 8 nodes, all-to-all · held fixed: drive 0.65, coupling 0.6 (neurolib default), no noise, 8 s window · only τ_inh varies -->
...
<!--
Three sources. The Anjum npj 2024 paper is where the data and the literature-based hypotheses come from, the same 49 control and 100 PD recordings. Wilson and Cowan 1972 is the original neural-mass model we use. neurolib is the Python simulation framework the model runs in.
-->
...
# Appendix: per-subject distributions and effect sizes

<div class="cols">
<div>

![w:550](figures/feature_boxplots.svg)

</div>
<div>

![w:530](figures/effect_forest.svg)

</div>
</div>

<!--
Two reference views of the same four features. Left, the full per-subject distributions: the box is the median and quartiles, the dots are individual subjects, with the alpha peak down in patients, theta up, beta down, and alpha power unchanged. Right, the standardised effect sizes: rank-biserial correlation with a 95 percent bootstrap confidence interval; alpha peak, theta, and beta sit well away from zero in the predicted direction, alpha power sits on zero.
-->
