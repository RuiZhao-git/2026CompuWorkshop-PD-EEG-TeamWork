...
- The point: the change is in frequency, not amplitude, so we say alpha slowing, not alpha loss.
-->
...
- Lower-left, the repository: numbered scripts, reusable modules, a results folder every run writes into.
  - Terminal: every number is the real output of one script, and each run writes the exact file shown.
- Lower-right, version control: branches merged into main, fifty commits; main reproduces every result and this deck.
-->
