...
T(140, 16, "EEG to result, through six numbered scripts", 13.5, MUTE, 400)
...
for kind, a, b, w in spine:
    ...
    if kind == "box":
        ...
        T(x + w / 2, cy - 3, a, 19, "#ffffff", 700, "middle")
        T(x + w / 2, cy + 14, b, 13, "#cfe2ff", 500, "middle")
    elif kind == "data":
        rrect(x, cy - 15, w, 30, 8, "#f1f5f9", LINE, 1)
        T(x + w / 2, cy + 4, a, 13.5, "#475569", 500, "middle", MONO)
    else:
        rrect(x, cy - 18, w, 36, 9, "#fdf1ec", ORANGE, 1.4)
        T(x + w / 2, cy + 5, a, 15, "#a8431f", 600, "middle")
    pos[a] = (x + w / 2, w)
    prev_right = x + w
    x += w + gap
...
T(hcx, hy + 15, "one number separates the two models:", 13, GREY, 400, "middle")
...
for anchor, num, label, direction in support:
    ...
    tw = 40 + len(label) * 6.5
    tx = ax - tw / 2
    rrect(tx, sup_y, tw, 24, 12, "#f8fafc", LINE, 1)
    raw(f'<circle cx="{tx+15}" cy="{sup_y+12}" r="8.5" fill="#e2e8f0"/>')
    T(tx + 15, sup_y + 16, num, 10.5, SLATE, 700, "middle")
    T(tx + 30, sup_y + 16.5, label, 12, GREY, 400)
...
T(24, 454, "every result is one script run", 13.5, GREY, 500)
...
T(tx + tw - 14, ty + 16, "bash", 11.5, MUTE, 400, "end", MONO)
...
def term(segs):
    global ln
    s = f'<text x="{tx+18}" y="{ln:.0f}" font-family="{MONO}" font-size="12.5">'
...
# =============================================================== ZONE B-right: GIT
T(612, 212, "VERSION CONTROL", 14.5, GREY, 500)
T(786, 212, "feature branches, reviewed, merged into main", 13, MUTE, 400)
...
T(612, main_y + 4, "main", 14.5, INK, 600, "start", MONO)
...
T(gx1 - 31, main_y - 16, "latest", 12.5, "#ffffff", 600, "middle")
...
def branch(name, color, ly, xs, xe, n, pr):
    ...
    # branch label
    T(a - 2, ly - 12, name, 13, color, 600)
    # merge tag near the merge point
    rrect(xe - 36, ly - 2, 66, 19, 9.5, "#ffffff", color, 1.2)
    T(xe - 3, ly + 11.5, "✓ merged", 11, color, 600, "middle")
