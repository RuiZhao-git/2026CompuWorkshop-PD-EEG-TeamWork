...
style: |
  ...
  .power { color: #92400e; font-weight: 600; }
  .peak  { color: #0f766e; font-weight: 600; }
...
Originally formulated as *"decreased alpha <span class="power">power</span>"*. Group comparison on 149 subjects showed that alpha <span class="power">power</span> is **essentially unchanged** between groups, while the alpha <span class="peak">peak frequency</span> **drops by ~1 Hz in PD** (median 9.25 → 8.00 Hz, p < 0.001).
The wording was refined to *"alpha <span class="peak">peak</span> slowing"* to reflect the actual change in the data.
...
Each **node** is one small piece of cortex, containing two neural populations: an excitatory group (**E**) and an inhibitory group (**I**). Their interaction produces an oscillation that looks like the resting alpha rhythm we measure on the scalp. We connect **N = 8** such nodes in a network where every node is connected to every other.

## The knob

The model has one parameter that controls how fast the oscillation runs: **`τ_inh`**, the time inhibitory neurons need to respond. Smaller `τ_inh` gives a faster rhythm, larger `τ_inh` gives a slower one. This is the knob we turn to simulate the alpha slowing our hypothesis predicts.

## Setup

- All 8 nodes share **the same parameters** (matches our whole-cortex assumption)
- **All-to-all** coupling between nodes, global strength `K_gl`
- No noise, so the output depends only on `τ_inh`
