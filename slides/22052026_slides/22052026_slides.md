...
## Validation gap: per-subject fitting has not been sanity-checked
...
A standard validation step we have not done:

1. Generate synthetic time series from the Wilson-Cowan model at a **known** τ_inh.
2. Run the same feature extraction and fitting pipeline on those synthetic signals.
3. Check whether the fitted τ_inh matches the value we put in.
...
We would like input from the podium on how to design this validation in a way that is informative without growing the project beyond its current scope.
